...
public:
    ...
    void init(ActivationFunction AFN, TaskType TT, int InputNeurons, std::vector<int> HiddenNeurons, int OutputNeurons){

        outputLayer.init(TT, HiddenNeurons[HiddenNeurons.size()-1], OutputNeurons);
        for(int i = 0; i < HiddenNeurons.size(); i++){
            if(i == 0){
                HiddenLayer hiddenlayer(AFN, InputNeurons, HiddenNeurons[i]);
                HiddenLayers.push_back(hiddenlayer);
    ...
    void load(std::ifstream& fileIn, bool deep){
